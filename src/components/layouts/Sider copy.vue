...
<script>
	...
	export default {
		...
		computed: {
			...
			...stateStore.mapGetters([{currentPathx: 'currentPath'}]),
		},
		created() {
			// console.log('currentPath', this.currentPath)
			console.log('menusx', viewRoutes)

			let openKey = '', selectedKey = '', keys = [];
			keys = viewRoutes.filter(item => item.meta.depth == 1).filter(item => new RegExp('^'+ this.createRegExp(this.currentPath)).test(item.path));
			openKey = keys[0].path.split('/')[2];
		...
		methods: {
			...
			createRegExp(path) {
				let regPath = path.split('/');
				if (regPath.length > 3) {
					regPath = regPath.splice(0, 3);
					return regPath.join('/')
				}
				return path;
			}
