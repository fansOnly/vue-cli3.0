	...
	<div>
		...
		<div class="container">
            ...
            <a-table rowKey="id" :loading="loading" :columns="columns" :dataSource="memberList" :pagination="pagination" :rowSelection="rowSelection" bordered @change="handleChange" >
                <span slot="avatarSlot" slot-scope="action">
...
<script>
    ...
    import { MEMBER_STATUS } from '@/utils/status';
        ...
        methods: {
			...
			delMultiItems() {
				...
				this.$confirm({
					...
					okType: 'danger',
					onOk() {
						that.memberList = that.memberList.filter(
            ...
            showModal (action, editId) {
                if (action == 'add') {
                    ...
                    this.editId = editId;
                    this.action = action;
                    this.modalTitle = this.allowEdit ? '编辑会员信息' : '查看会员信息';
